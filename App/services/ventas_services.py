...
def crear_venta_service(data, session):
    ...
    for item in data.detalles:
        ...
        existencias = session.exec(select(ProductoAlmacen).where(ProductoAlmacen.producto_id == item.producto_id, ProductoAlmacen.cantidad > 0).order_by(ProductoAlmacen.cantidad.desc())).all()
        ...
        detalles_db.append(DetalleVenta(venta=nueva_venta, producto_id=item.producto_id, cantidad=item.cantidad, precio_unitario=item.precio_unitario, subtotal=item.cantidad * item.precio_unitario))
